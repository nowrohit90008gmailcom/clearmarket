...
def build_user_response(user: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "analyses_limit": PLANS.get(plan_id, PLANS["free"])["analyses_limit"],
...
def resolve_symbol(symbol: str) -> Dict[str, str]:
    ...
    for stock in STOCK_UNIVERSE:
        if stock["symbol"] == base:
            return stock
    return {"symbol": base, "name": base, "exchange": "NSE", "yf": base if "." in base else f"{base}.NS"}
...
def build_price_history(hist) -> List[Dict[str, Any]]:
    return [{"date": idx.strftime("%Y-%m-%d"), "price": round(safe_float(row.get("Close")), 2)} for idx, row in hist.tail(30).iterrows()]
